...
function Form(props){
    ...
    function onFormSubmit(e){
        ...
        setCurrUserAge(``)
        setCurrUserName(``)
        props.onSaveNewUser({id:Math.random().toString(),username: currUserName,userage:currUserAge})                       
    }


    return <Card> <form onSubmit={onFormSubmit} className={styles.form}>
        <label >Username</label>
        <input value={currUserName} type="text" name='userName' onChange={onUserNameChange}/>
        <label >Age (years)</label>
        <input value={currUserAge} type="number" name='age' onChange={onUserAgeChange}/>
